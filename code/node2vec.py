...
WPG = nx.read_edgelist("../data/weighted_graph_paper.edgelist",
                       nodetype=int, data=(("weight", float),))

# read collaboration paper graph
PG = nx.read_edgelist("../data/graph_paper.edgelist",
                      nodetype=int, data=(("weight", float),))

# node2vec WG
edges_list = [(str(e[0]), str(e[1]), WG[e[0]][e[1]]["weight"])
              for e in WG.edges]
g = Graph(edges_list, directed=False, weighted=True)
node2vec = Node2Vec(g, dim=64, walk_length=15,
...
# node2vec WPG
edges_list = [(str(e[0]), str(e[1]), WPG[e[0]][e[1]]["weight"])
              for e in WPG.edges]
g = Graph(edges_list, directed=False, weighted=True)
node2vec = Node2Vec(g, dim=64, walk_length=15,
                    context=10, p=1, q=0.5, workers=10)
